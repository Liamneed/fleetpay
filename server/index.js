...
app.post('/api/admin/launch-reset',adminAuth,requireStaffRole('administrator'),(req,res)=>{
 ...
 try{
  ...
  db.exec('BEGIN IMMEDIATE');
  for(const table of ['customer_payments','settlement_runs','payouts','payment_requests','carried_charges','driver_notifications','driver_ledger','payout_runs','autocab_adjustments','fee_ledger','communications_log','early_summary_notifications','auth_challenges','demo_state','driver_cache','audit_logs'])db.exec(`DELETE FROM ${table}`);

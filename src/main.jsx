...
function AdminApp(){
 ...
 async function refreshCore(){await Promise.all([loadOverview(),loadTransactions(),loadDrivers(),loadSettings(),loadIntegrations(),loadSett(),loadMonday(),loadOutstanding(),loadFees(),loadEarlySummary()])}
 useEffect(()=>{
  let alive=true;
  ...
  (async()=>{
   try{
    ...
    await Promise.all([loadOverview(),loadTransactions(),loadDrivers(),loadSettings(),loadIntegrations(),loadSett(),loadMonday(),loadOutstanding(),loadFees(),loadEarlySummary()]);
...
function DriverApp(){
 ...
 const HomePage=()=> <div className="driverPageView driverHomeVNext">
  {mondayPriority&&PaymentDueCard({hero:true})}
  <section className={`driverHeroBalance balanceState-${balanceState}`}>
   ...
   <div className="balanceStateStrip"><span>{balance>0?'Money due to you':balance<0?'Money you need to pay':'Account clear'}</span><b>{balance>0?`${money(me.availableForEarlyPayout)} available now`:balance<0?(paymentRequests.length?'Payment requested':'Will carry forward unless requested'):'Up to date'}</b></div>
   <div className="heroBalanceActions"><button type="button" onClick={()=>changeTab('pay')}><CreditCard/>Take payment</button><button type="button" onClick={()=>changeTab(balance<0&&paymentRequests.length?'pay':'pay')}><ArrowUpRight/>{balance<0&&paymentRequests.length?'Pay balance':'Early payout'}</button></div>
  ...
  <div className="glanceGrid">
   ...
   <button className="glanceCard payout" onClick={()=>changeTab('pay')}><ArrowUpRight/><span>Early payout</span><b>{money(me.availableForEarlyPayout)}</b><small>{me.earlyPayoutAllowed?'Available now':'Check payout window'}</small></button>
